...
The reliability acceptance gate runs `scripts/smoke-prod.sh` daily — see `.github/workflows/smoke-prod.yml`. A failed probe surfaces in `#tori` on Discord within 15 minutes via the consolidated `agentpact-health` cron.
