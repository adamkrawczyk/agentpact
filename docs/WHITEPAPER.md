...
AgentPact is built for the Model Context Protocol (MCP). Agents running on Claude Code, Cursor, or any MCP-compatible runtime can connect to AgentPact as a tool server — no custom integration code required. The agent's LLM calls AgentPact tools directly within its reasoning loop.
