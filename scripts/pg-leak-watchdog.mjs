 ...
 * Supabase Postgres, run as a Hermes cron via `railway run -s @agentpact/api`
 * (which injects DATABASE_URL — no credential stored on disk).
 *
 ...
 *
 * Output protocol (matches postgres-leaked-tx-watchdog skill):
 *   clean   -> "HEARTBEAT_OK pg-leak-watchdog | agentpact-supabase: clean"
 *   action  -> above + a "=== TERMINATIONS ===" block (cron LLM escalates on 3rd+ fire/30min)
 */
