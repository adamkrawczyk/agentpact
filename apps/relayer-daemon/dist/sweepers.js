 ...
 */
export async function runAckTimeoutSweep(sql, chain, now = new Date()) {
    ...
    LIMIT 100
  `;
    const result = { scanned: rows.length, acted: 0, failed: [] };
 ...
 */
export async function runSchellingSweep(sql, chain, now = new Date()) {
    const rows = await sql `
 ...
 */
export async function runStreamStaleSweep(sql, now = new Date()) {
...
//# sourceMappingURL=sweepers.js.map