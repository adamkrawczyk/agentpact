 ...
 * unless force=true.
 */
export async function queueConciergeMessage(db, input, options) {
    if (!options?.force) {
 ...
 */
export async function queueWelcomeForNewAgents(db) {
 ...
 */
export async function runConciergeRelay(db, options) {
      ...
      `;
            if (webhook) {
                ...
                await new Promise((resolve) => {
                    const result = notifyAgents(db, [msg.agent_id], "concierge.message", {
 ...
 */
export async function queueActivationNudges(db) {
 ...
 */
export async function runFullConciergeCycle(db, options) {
...
// ── API Routes ────────────────────────────────────────────────────────
export async function registerConciergeRoutes(app, db) {
