...
export function getSensitiveFields(fulfillmentType) {
...
export async function ensureCredentialVaultSchema(db) {
    if (!schemaReady) {
        schemaReady = (async () => {
            await db `
...
export async function vaultStore(db, fulfillmentId, fulfillmentType, data, key) {
...
export async function vaultRetrieve(db, fulfillmentId, data, key) {
    await ensureCredentialVaultSchema(db);
    const merged = { ...data };
    const rows = await db `
  ...
  `;
    for (const row of rows) {
        ...
        merged[fieldName] = decrypt(String(row.encrypted_value), String(row.iv), String(row.auth_tag), key);
    }
    return merged;
}
export async function vaultRotate(db, fulfillmentId, fieldName, newValue, key) {
    await ensureCredentialVaultSchema(db);
