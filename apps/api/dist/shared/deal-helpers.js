...
import { decrypt, ensureCredentialVaultSchema, encrypt, getSensitiveFields, } from "../credential-vault.js";
...
let _credentialEncryptionKey = null;
function getCredentialEncryptionKeyLazy() {
    if (!_credentialEncryptionKey) {
        const { getCredentialEncryptionKey } = require("../credential-vault.js");
        _credentialEncryptionKey = getCredentialEncryptionKey();
    }
    return _credentialEncryptionKey;
}
export async function storeBuyerContext(fulfillmentId, fulfillmentType, data, credentialEncryptionKey) {
    const vaultSql = sql;
...
export async function retrieveBuyerContext(fulfillmentId, data, credentialEncryptionKey) {
    const vaultSql = sql;
    await ensureCredentialVaultSchema(vaultSql);
    const merged = { ...data };
...
export async function logCredentialAccess(fulfillmentId, agentId, action, ipAddress) {
    const vaultSql = sql;
    await ensureCredentialVaultSchema(vaultSql);
    await sql `
...
export async function applyFulfillmentExpiryChecks(deal, fulfillment) {
...
export async function enforceDealDelta(dealId, negotiatedTotal) {
...
export async function releaseMilestonePayment(milestoneId) {
  ...
  `;
    ...
    await sql.begin(async (txn) => {
        await txn.unsafe(`UPDATE payment_intents SET status = 'released', released_at = NOW(), updated_at = NOW(), tx_hash = $1 WHERE id = $2`, [`sim_release_${randomUUID().slice(0, 8)}`, payment.id]);
        await txn.unsafe(`UPDATE milestones SET status = 'accepted', accepted_at = NOW() WHERE id = $1`, [milestoneId]);
        await txn.unsafe(`UPDATE deals SET status = 'completed', updated_at = NOW() WHERE id = (SELECT deal_id FROM milestones WHERE id = $1)`, [milestoneId]);
        await txn.unsafe(`UPDATE offers SET status = 'archived', updated_at = NOW()
       WHERE id = (SELECT offer_id FROM deals WHERE id = (SELECT deal_id FROM milestones WHERE id = $1)) AND status = 'active'`, [milestoneId]);
        await txn.unsafe(`INSERT INTO audit_log (action, object_type, object_id, payload_json)
       VALUES ('payment.release', 'milestone', $1, $2::jsonb)`, [milestoneId, JSON.stringify({ gross, sellerAmount, feeAmount, platformWallet: PLATFORM_WALLET })]);
    });
...
export async function completeDealMilestones(dealId, opts = {}) {
  ...
  `;
    ...
    `;
        ...
        `;
                ...
                console.error(`[completeDealMilestones] audit_log insert failed for ${dealId}: ${auditErr instanceof Error ? auditErr.message : String(auditErr)}`);
            }
            return {
                mode,
...
export async function createDealProposal(proposal, opts) {
    const isFreeTier = isZeroPrice(proposal.negotiatedTotal);
    const result = await sql.begin(async (txn) => {
        const [deal] = await txn.unsafe(`INSERT INTO deals (
        ...
        for (const milestone of proposal.milestones) {
            ...
            const [ms] = await txn.unsafe(`INSERT INTO milestones (deal_id, idx, title, amount, currency, acceptance_criteria, due_at, status)
        ...
        await txn.unsafe(`INSERT INTO negotiation_events (deal_id, actor_agent_id, event_type, payload_json)
        VALUES ($1, $2, 'propose', $3::jsonb)`, [deal.id, opts.negotiationActorAgentId, JSON.stringify(opts.auditPayload ?? proposal)]);
