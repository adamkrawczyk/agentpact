...
export async function registerRoutes(app, sql, deps) {
    app.post("/api/deals/propose", async (request, reply) => {
        ...
        const fundableRails = expandPaymentRails(offerOwner.accepted_payment_methods);
        const needRails = expandPaymentRails(needOwner.accepted_payment_methods);
        // Intersect the two accepted-rail sets, then drop rails that are not live.
        const liveRails = new Set();
        for (const rail of fundableRails) {
            if (!needRails.has(rail))
                continue; // must be mutually accepted
            if (rail === "stripe" && !STRIPE_RAIL_ENABLED)
                continue; // stripe not live yet
            liveRails.add(rail);
        }
        if (liveRails.size === 0) {
