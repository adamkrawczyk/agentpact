        ...
        try {
            ...
            if (missingTexts.length > 0) {
                ...
                for (let i = 0; i < missingRefs.length; i += EMBED_WRITE_BATCH) {
                    ...
                    await Promise.all(chunk.map((ref, j) => {
    ...
    for (const offer of offers) {
        for (const need of needs) {
            ...
            if (!semanticEnabled) {
                ...
                if (semanticScore !== null && overlap.length === 0 && semanticScore <= 0.75)
                    continue;
...
export async function registerRoutes(app, sql, deps, recomputeMatchesInput = createRecomputeMatchesQueue(() => recomputeMatches(app, sql))) {
    ...
    app.post("/api/embeddings/recompute", async (request, reply) => {
        ...
        for (let i = 0; i < offers.length; i += EMBED_BATCH_SIZE) {
            ...
            await sql.begin(async (txn) => {
                for (let j = 0; j < batch.length; j += 1) {
                    await txn.unsafe(`UPDATE offers SET description_embedding = $1::jsonb WHERE id = $2`, [JSON.stringify(offerEmbeddings[i + j]), batch[j].id]);
                }
            });
        ...
        for (let i = 0; i < needs.length; i += EMBED_BATCH_SIZE) {
            ...
            await sql.begin(async (txn) => {
                for (let j = 0; j < batch.length; j += 1) {
                    await txn.unsafe(`UPDATE needs SET description_embedding = $1::jsonb WHERE id = $2`, [JSON.stringify(needEmbeddings[i + j]), batch[j].id]);
                }
            });
            totalUpdated += batch.length;
        }
