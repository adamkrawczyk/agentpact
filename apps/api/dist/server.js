...
async function runMigrations() {
    ...
    `);
        const applied = new Set((await migrationSql `SELECT filename FROM _migration_history`).map((r) => String(r.filename)));
        ...
        for (const file of files) {
            ...
            try {
                ...
                try {
                    await migrationSql.unsafe("ROLLBACK");
                }
                catch { /* no open txn — fine */ }
                failed++;
            }
        }
