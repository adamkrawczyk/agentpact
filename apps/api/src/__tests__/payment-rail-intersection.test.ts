...
// payment-methods rolloutb — rail-intersection enforcement (matching + propose gate).
...
describe("propose rail-intersection gate (payment-methods rolloutb)", () => {
  let buyerId: string;
  let sellerId: string;
...
// payment-methods rolloutb — autopilot bypass + stale-match gate.
...
describe("autopilot rail-intersection gate (payment-methods rolloutb)", () => {
  let buyerId: string;
  let sellerId: string;
  let buyerHeaders: Record<string, string>;
  let sellerHeaders: Record<string, string>;
