...
async function runMigrations() {
  ...
  if (!migrationsDir) {
    ...
    throw new Error(
      `[migrations] FATAL: no migrations directory found. Looked in: ${candidates.join(", ")}`,
    );
  }
  ...
  if (failures.length > 0) {
    ...
    throw new Error(
      `[migrations] FATAL: ${failures.length} migration(s) failed — refusing boot.\n${detail}`,
    );
  }
}

function shouldRunMigrationsAtBoot(): boolean {
