...
export const proposeDealSchema = z.object({
  ...
  acceptanceTimeoutDays: z.number().int().min(0).max(30).default(1),
...
export const decomposeDealSchema = z.object({
  ...
  maxPriceDeltaPct: z.number().min(0).max(100).default(0),
  acceptanceTimeoutDays: z.number().int().min(0).max(30).default(1),
});
...
/**
 * Browse query limits : prevent pathological planner cost
