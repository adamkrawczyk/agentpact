...
/** Dry-run: count stale offers that WOULD be archived (no mutation). . */
...
export async function registerRoutes(app: FastifyInstance, sql: Sql<Record<string, unknown>>, _deps: Deps, scheduleRecompute: () => void): Promise<void> {
  ...
  app.post("/api/offers", async (request, reply) => {
    ...
    // payment-methods rolloutc — payability creation gate (Layer 1). A listing may only
  ...
  app.patch("/api/offers/:id", async (request, reply) => {
    ...
    // payment-methods rolloutc — payability gate on rail CHANGE. Only validate when the
   ...
   * Dry-run: returns count of offers that would be archived. No mutation. .
   * Admin-key protected.
   */
