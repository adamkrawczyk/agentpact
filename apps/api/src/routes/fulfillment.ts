...
export async function registerRoutes(app: FastifyInstance, sql: Sql<Record<string, unknown>>, deps: Deps): Promise<void> {
  ...
  app.post("/api/deals/:id/fulfillment", async (request, reply) => {
    ...
    if (!dealFull?.is_free_tier && Number(dealFull?.acceptance_timeout_days ?? 1) === 0) {
      // Move the fulfillment UPDATE inside the try so it can be rolled back on failure
      try {
        const releaseResult = await completeDealMilestones(id, { skipOnChainRelease: false });
        // payment-methods rollout — only celebrate (reputation bump, "Deal complete!"
        ...
        // Do not silently fall through — report the inconsistency
        return reply.code(500).send({
          error: "Auto-complete failed — deal may be in an inconsistent state",
          detail: autoErr.message,
          dealId: id,
        });
      }
    }
  ...
  app.post("/api/deals/:id/confirm-delivery", async (request, reply) => {
    ...
    // payment-methods rollout — if the deal was held (no real-money funding), do not
  ...
  app.post("/api/deals/:id/close", async (request, reply) => {
    ...
    try {
      ...
      // payment-methods rollout — held deal (unfunded milestones): do not archive,
  ...
  app.post("/api/deals/:id/fulfillment/auto-complete", async (request, reply) => {
    ...
    const timeoutDays = Number(deal.acceptance_timeout_days ?? 1);
