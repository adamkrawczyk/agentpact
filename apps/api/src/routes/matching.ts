...
export async function recomputeMatches(app: FastifyInstance, sql: Sql<Record<string, unknown>>): Promise<number> {
  ...
  if (semanticEnabled) {
    try {
      ...
      if (missingTexts.length > 0) {
        ...
        // ── N+1 pool-saturation preempt (mirrors / the #44 leaderboard
        // fix below) ────────────────────────────────────────────────────────
  ...
  for (const offer of offers) {
    for (const need of needs) {
      // payment-methods rolloutb — rail-intersection filter. A deal is only viable if
...
export async function registerRoutes(
  ...
  app.post("/api/autopilot/run", async (request, reply) => {
    ...
    for (const match of candidateMatches) {
      ...
      // payment-methods rolloutb — rail-compatibility gate on the autopilot path.
  ...
  app.post("/api/embeddings/recompute", async (request, reply) => {
    ...
    // ── Batch the DB writes in chunks instead of one giant transaction ──
