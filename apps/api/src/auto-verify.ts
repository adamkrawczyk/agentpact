export async function autoVerify(type: string, data: Record<string, unknown>): Promise<{ success: boolean; details: string }> {
  switch (type) {
    case "http-ping": {
    ...
    case "web-scrape-leads-v1": {
      ...
      const requiredColumns: string[] = Array.isArray((data as any).spec?.required_columns)
        ? (data as any).spec.required_columns
        : [];
      const minRows: number = typeof (data as any).spec?.min_rows === "number"
        ? (data as any).spec.min_rows
        : 0;
    ...
    case "transcribe-audio-v1": {
      ...
      const expectedFormat: string = typeof spec.format === "string" ? spec.format.toLowerCase() : "";
      const minLengthChars: number = typeof spec.min_length_chars === "number" ? spec.min_length_chars : 0;
      const mustContainKeywords: string[] = Array.isArray(spec.must_contain_keywords) ? spec.must_contain_keywords : [];
      ...
      try {
        ...
        if (expectedFormat) {
          ...
          const formatMatches =
            contentType.includes(expectedFormat) ||
    ...
    case "classify-rows-v1": {
      ...
      const spec = (data as any).spec ?? {};
      const requiredColumns: string[] = Array.isArray(spec.required_columns) ? spec.required_columns : [];
      const minRows: number = typeof spec.min_rows === "number" ? spec.min_rows : 0;
      const expectedFormat: string = typeof spec.format === "string" ? spec.format.toLowerCase() : "csv";
      const minConfidenceAvg: number | null = typeof spec.min_confidence_avg === "number" ? spec.min_confidence_avg : null;
        ...
        if (expectedFormat === "json") {
          ...
          if (minConfidenceAvg !== null && headers.includes("confidence")) {
            const confValues = rows
              .map(r => Number((r as Record<string, unknown>)["confidence"]))
