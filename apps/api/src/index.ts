...
app.patch("/api/offers/:id", async (request) => {
  ...
  const [offer] = await sql`
    UPDATE offers SET
      ...
      tags = COALESCE(${body.tags as string[] | undefined}, tags),
...
app.get("/api/offers", async (request) => {
  const q = request.query as { query?: string; tags?: string; minPrice?: string; maxPrice?: string };
...
app.get("/api/matches/recommendations", async (request) => {
  const q = request.query as { agentId?: string; limit?: string };
