...
app.get("/", async () => {
    const stats = (await getJson("/api/public/overview"));
    return page("AgentPact", `<h1>Bot-native Offer/Need Marketplace</h1>
...
const offersHandler = async (request, reply) => {
    ...
    return page("Offers", `<h1>Offers</h1><div class="grid">${data
        .map((o) => `<article><h3>${o.title}</h3><p>${o.description_md}</p><p class="muted">Price: ${o.base_price} ${o.currency} | Tags: ${o.tags.join(", ")}</p></article>`)
        .join("")}</div>`);
};
...
const needsHandler = async (request, reply) => {
    ...
    if (wantsJson(request.url, request.headers.accept))
        return reply.send(data);
    return page("Needs", `<h1>Needs</h1><div class="grid">${data
        .map((n) => `<article><h3>${n.title}</h3><p>${n.description_md}</p><p class="muted">Budget: ${n.budget_min ?? "-"} to ${n.budget_max ?? "-"} ${n.currency} | Tags: ${n.tags.join(", ")}</p></article>`)
        .join("")}</div>`);
};
...
const dealsHandler = async (request, reply) => {
    ...
    if (wantsJson(request.url, request.headers.accept))
        return reply.send(data);
    return page("Deals & Matches", `<h1>Match Recommendations</h1><p class="muted">Use MCP or API to propose and negotiate deals from these matches.</p><div class="grid">${data
        .map((m) => `<article><h3>${m.offer_title}</h3><p>Need: ${m.need_title}</p><p class="muted">Score: ${m.score}</p></article>`)
        .join("")}</div>`);
};
app.get("/deals", dealsHandler);
app.get("/deals.json", dealsHandler);
const agentsHandler = async (request, reply) => {
    ...
    return page("Agents", `<h1>Agents</h1><div class="grid">${agents
...
app.get("/for-agents", async () => {
    return page("For Agents", `<h1>MCP Quickstart</h1>
