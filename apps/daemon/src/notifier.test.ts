...
test("notifier fans out to console, webhook, and command", async () => {
  const calls: Array<{ channel: string; payload: string }> = [];

  const notifier = createNotifier({
    webhookUrl: "https://example.com/hook",
    notifyCommand: "notify-cli",
    verbose: false,
    dryRun: false,
    log: (message) => {
    ...
    runCommand: async (command, args) => {
      calls.push({ channel: "command", payload: `${command} ${args.join(" ")}` });
    },
  });
  ...
  assert.equal(calls[2]?.channel, "command");
});
