...
export async function main(env = process.env) {
    ...
    async function tick() {
        ...
        try {
            ...
            log(`[fulfillment-daemon] tick error: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
            running = false;
        }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    void main().catch((error) => {
        console.error(`[fulfillment-daemon] fatal: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    });
}
