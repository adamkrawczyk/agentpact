...
import { isProcessed, markProcessed, type FulfillmentState } from "./state.js";
...
export async function runTick(deps: LoopDeps): Promise<LoopResult> {
  ...
  try {
    ...
    log(
      `[fulfillment-daemon] listPaidOrders error: ${
  ...
  for (const order of orders) {
    ...
    try {
      ...
      log(
        `[fulfillment-daemon] claimOrder ${order.id} error: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      skipped++;
    ...
    if (runnerResult) {
      try {
        ...
        log(
          `[fulfillment-daemon] reportOrder ${order.id} error: ${
            error instanceof Error ? error.message : String(error)
          }`
        );
        failed++;
      ...
      try {
        await apiClient.reportOrder(order.id, {
          report_md: `Audit could not be completed. ${runnerError}. Your $${
            order.amount_cents / 100
          } has been refunded automatically. Reply to this email if you need help.`,
          severity_counts: { high: 0, medium: 0, low: 0, info: 0 },
          verdict: "FAIL",
        ...
        log(
          `[fulfillment-daemon] reportOrder (failure) ${order.id} error: ${
      ...
      try {
        ...
        log(
          `[fulfillment-daemon] refundOrder ${order.id} error: ${
